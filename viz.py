"""
This file contains some boilerplate code that I use all the time.

It makes plotting with xarray much much easier.
Also changes some of the default settings in matplotlib

Example
-------
ds = xr.open_dataset('<path_to_netcdf.nc>')
da = ds.some_variable_name.mean('time')
da.geo()
...
try:
    ...
    crs = None
    warnings.warn(
        'You will not be able to use the xr.DataArray.geo() function '
        'as cartopy is not installed. ')
...
rcMaps = {
    ...
    "proj": None if crs is None else crs.PlateCarree(central_longitude=205),
    "land_color": "none",
    "coast_res": "110m",
    "coast_lw": 0.5,
    "robust": True,
